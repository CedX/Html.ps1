...
Describe "New-CustomElement" {
	It "should create a custom HTML element from the specified tag name" {
		Should-BeString "<my-element></my-element>" (New-HtmlCustomElement my-element) -CaseSensitive
	}

	It 'should handle the "id" attribute' {
		Should-BeString '<my-element id="foo"></my-element>' (New-HtmlCustomElement my-element -Id foo) -CaseSensitive
	}

	It 'should handle the "class" attribute' {
		Should-BeString '<my-element class="btn btn-danger"></my-element>' (New-HtmlCustomElement my-element -Class btn, btn-danger) -CaseSensitive
		Should-BeString '<my-element class="btn btn-info btn-sm"></my-element>' (New-HtmlCustomElement my-element -Class "btn btn-info", btn-sm) -CaseSensitive
	}

	It 'should handle the "style" attribute' {
		$expected = '<my-element style="font-family: &quot;Segoe UI&quot;; font-size: 1rem"></my-element>'
		Should-BeString $expected (New-HtmlCustomElement my-element -Style ([ordered]@{ FontFamily = '"Segoe UI"'; FontSize = "1rem" })) -CaseSensitive
	}

	It 'should handle the "tabindex" attribute' -ForEach -1, 0 {
		Should-BeString "<my-element tabindex=""$_""></my-element>" (New-HtmlCustomElement my-element -TabIndex $_) -CaseSensitive
	}

	It 'should handle the "title" attribute' -ForEach "", 'A "custom" label.' {
		Should-BeString ($_ ? '<my-element title="A &quot;custom&quot; label."></my-element>' : "<my-element></my-element>") (New-HtmlCustomElement my-element -Title $_) -CaseSensitive
	}
	...
	It "should handle custom attributes" {
		...
		$expected | Should-ContainCollection (New-HtmlCustomElement my-element -Attributes @{ "data-foo" = '"bar"'; disabled = $false; required = $true })
	}
	...
	It "should handle data attributes" {
		...
		$expected | Should-ContainCollection (New-HtmlCustomElement my-element -DataSet @{ BsToggle = "tooltip"; PushUrl = $true })
	}
	...
	It "should handle event handler attributes" {
		...
		$expected | Should-ContainCollection (New-HtmlCustomElement my-element -On @{ Click = "submit(event)"; ContextMenu = "showMenu()" })
	}
	...
	It "should handle the inner content" {
		...
		Should-BeString $expected (New-HtmlCustomElement outer-element { New-HtmlCustomElement inner-element { "Foo &gt; Bar"; " "; span "Baz &lt; Qux" } }) -CaseSensitive
	}
}
